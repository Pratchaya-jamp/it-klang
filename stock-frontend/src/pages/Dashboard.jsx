...
const ItemModal = ({ isOpen, onClose, onSuccess, initialData = null, categories = [] }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (initialData) {
        // จัดการกรณี jobNo เป็น null ให้แสดงเป็นค่าว่าง
  ...
  const isFormValid = () => {
    // ยกเว้น jobNo ไม่ให้เป็น required field
    const { jobNo, quantity, ...requiredFields } = formData;
    const isRequiredValid = Object.values(requiredFields).every(val => val !== '' && val !== null);
    if (!isEditMode) return isRequiredValid && quantity !== '' && quantity !== null;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ส่ง null กลับไปถ้า jobNo เป็นค่าว่าง
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider ml-1">รหัสอุปกรณ์ {isEditMode && <span className="text-zinc-300 font-normal">(แก้ไขไม่ได้)</span>}</label>
            ...
            {/* ✅ เพิ่มช่องกรอก Job No */}
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider ml-1">รหัสงาน <span className="text-zinc-300 font-normal normal-case">(ไม่บังคับ)</span></label>
